  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.container}>
        {/* Header */}
        <div style={s.header}>
          <div style={s.logo}>FL</div>
          <div>
            <h1 style={s.title}>Fitness Levels</h1>
        ...
        {!selectedClient && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            ...
            {search.length >= 2 && (
              <div style={s.results}>
                {filteredClients.map((c) => {
                  ...
                  return (
                    <div key={c.id} style={s.resultCard} onClick={() => setSelectedClient(c.id)}>
                      ...
                      <span style={{ ...s.badge, background: LEVELS[lvl - 1]?.color || "#555" }}>
                        Lv {lvl}
                      </span>
                    </div>
                  );
                ...
                {filteredClients.length === 0 && (
                  <p style={{ textAlign: "center", color: "var(--muted)", padding: 20, fontSize: 14 }}>
            ...
            {search.length < 2 && (
              <p style={{ textAlign: "center", color: "var(--muted)", marginTop: 40, fontSize: 14 }}>
                Inserisci almeno 2 lettere per cercare
              </p>
            )}
          </div>
        )}
        ...
        {client && (
          <div style={{ animation: "fadeIn 0.3s ease" }}>
            <button onClick={() => { setSelectedClient(null); setSearch(""); }} style={s.back}>
              ← Torna alla ricerca
            </button>

            <div style={s.profileHeader}>
              <h2 style={s.profileName}>{client.name}</h2>
              <span style={{ ...s.badge, background: LEVELS[clientLevel - 1]?.color || "#555", fontSize: 16, padding: "6px 16px" }}>
                Lv {clientLevel}
              </span>
            </div>
            ...
            <div style={s.totalProgress}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700 }}>
                  {achievements.filter(a => a.clientId === client.id).length} / {ALL_SKILLS.length}
                </span>
              </div>
              <div style={s.progressTrack}>
                <div style={{
                  ...s.progressFill,
                  width: `${(achievements.filter(a => a.clientId === client.id).length / ALL_SKILLS.length) * 100}%`,
                  background: LEVELS[Math.min(clientLevel, 6)]?.color || "#555",
                }} />
              </div>
            </div>
                ...
                return (
                  <div key={l.level} style={{
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                      <span style={{ fontWeight: 700, color: l.color, fontSize: 15 }}>
                        Livello {l.level}
                      </span>
                      {isComplete && (
                        <span style={{ fontSize: 12, fontWeight: 700, color: l.color }}>✓ COMPLETATO</span>
                      )}
                      {!isComplete && !isLocked && (
                    ...
                    {l.skills.map((sk) => {
                      ...
                      return (
                        <div key={sk.id} style={{
                          ...s.skillRow,
                          background: ach ? l.color + "15" : "transparent",
                          borderColor: ach ? l.color + "40" : "var(--border)",
                        }}>
                          <div style={{
                            width: 22, height: 22, borderRadius: "50%",
                            background: ach ? l.color : "var(--bg3)",
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            {ach && (
                              <div style={{ fontSize: 11, color: "var(--muted)", marginTop: 1 }}>
                                {ach.trainer} · {formatDate(ach.date)}
                              </div>
                            )}
                          </div>
                        </div>
...
const css = `
  :root {
    --bg: #FAFAF8; --bg2: #F0EFEB; --bg3: #E5E3DD;
    --text: #1A1A18; --text2: #4A4A45; --muted: #8A8A82;
    --border: #D8D6CF; --accent: #2D5A27;
    --font: 'DM Sans', 'Segoe UI', system-ui, sans-serif;
  }
  @media (prefers-color-scheme: dark) {
    :root {
      --bg: #141413; --bg2: #1E1E1C; --bg3: #2A2A27;
      --text: #EDEDEB; --text2: #B0B0A8; --muted: #6E6E66;
      --border: #333330; --accent: #6B8F71;
    }
  }
