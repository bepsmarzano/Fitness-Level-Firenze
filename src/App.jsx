...
export default function App() {
  ...
  const [trainers, setTrainers, trainersLoaded] = useFirestoreDoc(
    "config",
    "trainers",
    INITIAL_TRAINERS
  );
  ...
  const loaded =
    clientsStore.loaded && achievementsStore.loaded && classesStore.loaded && trainersLoaded;
...
const [collapsedParticipants, setCollapsedParticipants] = useState([]);
  ...
  // ── Computed ──
  const getClientLevel = useCallback(
    (clientId) => {
      const clientAch = achievements.filter((a) => a.clientId === clientId);
      let level = 0;
      for (const lvl of LEVELS) {
        const completedSkills = lvl.skills.filter((s) =>
          clientAch.some((a) => a.skillId === s.id)
        );
        if (completedSkills.length === 3) level = lvl.level;
        else break;
      }
      return level;
    },
  ...
  const getSkillAchievement = useCallback(
    (clientId, skillId) => {
  ...
  const toggleSkill = async (clientId, skillId) => {
    ...
    if (existing) {
      ...
      const id = generateId();
      await achievementsStore.upsert(id, {
        id,
        clientId,
        skillId,
  ...
  const createClass = async () => {
    if (!className.trim() || classParticipants.length === 0) return;
    const id = generateId();
    await classesStore.upsert(id, {
      id,
      name: className.trim(),
      trainer: currentTrainer,
  ...
  const removeTrainer = async (name) => {
    ...
    if (currentTrainer === name)
      setCurrentTrainer(trainers.filter((t) => t !== name)[0] || "");
  };

  // ── Report Data ──
  const reportData = useMemo(() => {
    const clientProgress = clients
      .map((c) => {
    ...
    achievements.forEach((a) => {
      ...
      if (skill) {
        trainerStats[a.trainer].byLevel[skill.level] =
          (trainerStats[a.trainer].byLevel[skill.level] || 0) + 1;
      }
    });
      ...
      <header style={styles.header}>
        <div style={styles.headerInner}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              <h1 style={styles.headerTitle}>Fitness Levels</h1>
          ...
          <div style={styles.fadeIn}>
            <div style={styles.topBar}>
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => setModal("import")} style={styles.btnSecondary}>
                  <Icons.Upload /> Importa
                </button>
                <button onClick={() => setModal("addClient")} style={styles.btnPrimary}>
                  <Icons.Plus /> Aggiungi
                </button>
              </div>
            </div>
                    ...
                    <div style={styles.progressBarTrack}>
                      <div
                        style={{
                          ...
                          width: `${(totalSkills / ALL_SKILLS.length) * 100}%`,
                          background: LEVELS[Math.min(lvl, 5)]?.color || "#555",
                        }}
                      />
                    </div>
                    <button
            ...
            return (
              <div style={styles.fadeIn}>
                ...
                <div style={styles.detailHeader}>
                  <div>
                    ...
                    <p style={styles.detailSub}>
                      Iscritto dal {formatDate(client.createdAt)}
                    </p>
                  </div>
                ...
                <div style={styles.levelsGrid}>
                  {LEVELS.map((l) => {
                    const completed = l.skills.filter((s) =>
                      getSkillAchievement(client.id, s.id)
                    );
                    ...
                    return (
                      <div
                        ...
                        style={{
                          ...
                          opacity: l.level > lvl + 1 ? 0.45 : 1,
                        }}
                      >
                        <div style={styles.levelSectionHeader}>
                          <span
                            style={{ fontWeight: 700, color: l.color, fontSize: 14 }}
                          >
                            Livello {l.level}
                          </span>
                          {isComplete && (
        ...
        {view === "class" && (
          <div style={styles.fadeIn}>
            {!activeClass ? (
              ...
              <>
                <button onClick={() => setActiveClass(null)} style={styles.backBtn}>
                  ← Torna alle classi
                </button>
                <div style={styles.detailHeader}>
                  <div>
          ...
          <div style={styles.fadeIn}>
            ...
            <div style={styles.levelsOverview}>
              {LEVELS.map((l) => (
                <div
                  key={l.level}
                  style={{ ...styles.levelOverviewCard, borderTopColor: l.color }}
                >
                  <div
                    style={{
                      display: "flex",
                  ...
                  >
                    <span style={{ fontWeight: 800, fontSize: 20, color: l.color }}>
                      Lv {l.level}
                    </span>
                  ...
                  >
                    {clients.filter((c) => getClientLevel(c.id) >= l.level).length} /{" "}
                      ...
                      <div style={styles.barTrack}>
                        <div
                          style={{
                            ...
                            background: LEVELS[lvl - 1]?.color || "#777",
                          }}
                        />
                      </div>
                      <div
            ...
            <div style={styles.reportSection}>
                      ...
                      .map(([lvl, count]) => (
                        <span
                          ...
                          style={{
                            ...
                            borderRadius: 20,
                            background:
                              LEVELS[parseInt(lvl) - 1]?.color + "22",
                            color: LEVELS[parseInt(lvl) - 1]?.color,
                            fontWeight: 600,
                          }}
                        >
                          Lv{lvl}: {count}
              ...
              {Object.keys(reportData.trainerStats).length === 0 && (
                <p style={{ color: "var(--text-muted)", fontSize: 13 }}>
                  Nessuna abilità ancora assegnata
                </p>
              )}
            </div>
                ...
                {reportData.clientProgress.map((c, i) => (
                  ...
                  >
                    ...
                    <span style={{ flex: 1, textAlign: "center", fontSize: 13 }}>
                      {c.totalSkills} / {ALL_SKILLS.length}
                    </span>
                  </div>
                ))}
              </div>
          ...
          <div style={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            {modal === "import" && (
              <>
                ...
                >
                  Un nome per riga, oppure separati da virgola o punto e virgola.
                </p>
                <textarea
                  value={importText}
            ...
            {modal === "createClass" && (
              <>
                <h3 style={styles.modalTitle}>Nuova Classe</h3>
                <input
                  value={className}
                  onChange={(e) => setClassName(e.target.value)}
                  placeholder="Nome classe (es. WOD Mattina)"
                  style={styles.input}
                  autoFocus
                />
                ...
                <button
                  onClick={createClass}
                  disabled={!className.trim() || classParticipants.length === 0}
                  style={{
                    ...styles.btnPrimary,
                    width: "100%",
                    marginTop: 12,
                    opacity:
                      !className.trim() || classParticipants.length === 0
                        ? 0.5
                        : 1,
                  }}
                >
              ...
              <>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
                  <input
                    ...
                    placeholder="Nome trainer"
                    style={{ ...styles.input, flex: 1 }}
                  />
                  <button onClick={addTrainer} style={styles.btnPrimary}>
                    <Icons.Plus />
                  </button>
                </div>
                {trainers.map((t) => (
                  <div key={t} style={styles.trainerRow}>
                    ...
                    <button onClick={() => removeTrainer(t)} style={styles.deleteBtn}>
                      <Icons.Trash />
                    </button>
                  </div>
                ))}
                ...
                >
                  ...
                  <p style={{ fontSize: 12, color: "var(--text-muted)", marginBottom: 8 }}>
                    Attenzione: il reset cancellerà tutti i dati dal database.
                  </p>
                  <button
                    onClick={async () => {
                      if (
                        confirm("Sei sicuro? Tutti i dati verranno cancellati.")
                      ) {
